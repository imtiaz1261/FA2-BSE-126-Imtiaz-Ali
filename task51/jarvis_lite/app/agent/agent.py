...
class IntelligentAgent:
    ...
    def _route_query(self, query: str) -> tuple[str, Optional[str], float]:
        ...
        # Check for calculator keywords or mathematical symbols
        ...
        has_math_symbols = any(char in query for char in "+-*/%()^")
        
        # Route to calculator if keywords present AND symbols present, OR if query is mostly math
    ...
    def _generate_rag_response(self, query: str) -> Dict[str, Any]:
        """Generate response using RAG with LLM fallback."""
        rag_step = ExecutionStep(
            state=AgentState.GENERATING_RESPONSE,
            query=query,
            reasoning="Using RAG pipeline with LLM fallback"
        )
        self.execution_history.append(rag_step)
        
        # If no RAG service, try LLM fallback directly
        if not self.rag_service:
            logger.info("RAG service not available, using direct LLM fallback")
            try:
                answer = self._generate_llm_direct(query)
                return {
                    "answer": answer,
                    "reasoning": "Generated using LLM (no documents)",
                ...
                logger.exception(f"Direct LLM fallback failed: {e}")
                return {
                    "answer": f"I can't provide an answer: {e}",
                    "reasoning": "No RAG service and LLM fallback failed",
                    "tool_used": None,
                    "execution_steps": [step.to_dict() for step in self.execution_history],
                    "confidence": 0.0,
                }
        ...
        try:
            # Query with LLM fallback enabled
            result = self.rag_service.query(query, use_llm_fallback=True)
            
            # Check source type to determine confidence
            source_type = result.get("source_type", "documents")
            confidence = 0.85 if source_type == "documents" else 0.65  # Lower confidence for LLM fallback
            tool_used = "RAG (documents)" if source_type == "documents" else "RAG (LLM fallback)"
            
            return {
                "answer": result.get("answer", "No answer generated"),
                "reasoning": f"Generated using {tool_used}",
                "tool_used": tool_used,
    ...
    def _generate_llm_direct(self, query: str) -> str:
        """Generate answer directly using LLM (no RAG).
        
        Used when RAG service isn't available.
        Tries Gemini first (more reliable), then Groq.
        """
        try:
            ...
            raise Exception("No LLM API key configured (set GEMINI_API_KEY or GROQ_API_KEY)")
        except Exception as e:
            logger.exception(f"Direct LLM generation failed: {e}")
            raise
