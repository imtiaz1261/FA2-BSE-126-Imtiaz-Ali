...
export const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, label, helperText, errorText, autoGrow = false, maxRows = 6, id, onChange, style, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1.5">
        {label && (
          <label htmlFor={textareaId} className="text-meta font-medium text-ink dark:text-ink-dark">
            {label}
          </label>
        )}
        <textarea
          ...
          onChange={onChange}
          className={cn(
            "min-h-20 w-full resize-none rounded-control border border-border dark:border-border-dark bg-canvas dark:bg-canvas-dark text-ink dark:text-ink-dark text-body px-3 py-2 placeholder:text-ink/40 dark:placeholder:text-ink-dark/40 transition-colors focus-visible:border-accent-600 disabled:cursor-not-allowed disabled:opacity-60",
