...
export function ShareDialog({
  ...
  return (
    <Modal open={open} onClose={onClose} title="Share conversation">
      <p className="mb-4 text-body text-ink/70 dark:text-ink-dark/70">
        ...
        They won't see your other conversations or be able to reply.
      </p>
      ...
      {shareUrl ? (
        <div className="flex flex-col gap-3">
          ...
          <Button variant="ghost" onClick={handleRevoke} disabled={isLoading} className="self-start">
            Stop sharing
          </Button>
        </div>
      ) : (
        <Button variant="primary" onClick={handleCreateLink} loading={isLoading}>
          Create share link
        </Button>
      )}
    </Modal>
