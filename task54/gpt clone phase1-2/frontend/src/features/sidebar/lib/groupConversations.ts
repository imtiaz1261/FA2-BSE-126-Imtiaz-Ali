import type { ConversationSummary, DateGroup } from "@/lib/conversationsApi";
...
const DATE_GROUP_ORDER: DateGroup[] = ["today", "yesterday", "previous_7_days", "older"];
 ...
 */
export function buildSidebarRows(items: ConversationSummary[]): SidebarRow[] {
  const rows: SidebarRow[] = [];

  const pinned = items.filter((c) => c.pinned);
  if (pinned.length > 0) {
    rows.push({ kind: "header", key: "group-pinned", label: "Pinned" });
    for (const c of pinned) rows.push({ kind: "item", key: c.id, conversation: c });
  }
  ...
  for (const group of DATE_GROUP_ORDER) {
    ...
    for (const c of inGroup) rows.push({ kind: "item", key: c.id, conversation: c });
  }
