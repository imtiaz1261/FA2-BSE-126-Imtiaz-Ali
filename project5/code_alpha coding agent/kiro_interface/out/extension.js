  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    ...
    switch (msg.type) {
      case "run":
        ...
        break;
      case "saveFile":
        await onSaveFile(msg.filePath, msg.code);
        break;
      case "openFile":
        await onOpenFile(msg.filePath);
        break;
    }
  });
  console.log("[Kiro] Panel opened with message handler registered");
}
async function onRun(prompt, repoPath) {
  var _a, _b;
  ...
  const ws = ((_b = (_a = vscode.workspace.workspaceFolders) == null ? void 0 : _a[0]) == null ? void 0 : _b.uri.fsPath) ?? repoPath;
  ...
  await consumeSSE(`${API}/tasks/${taskId}/stream`, (data) => {
    try {
...
async function onSaveFile(filePath, code) {
  try {
...
function send(msg) {
  panel == null ? void 0 : panel.webview.postMessage(msg);
}
...
function putJson(baseUrl, filePath, code) {
  return Promise.resolve();
}
function consumeSSE(url, onData) {
...
<body>
    ...
    <div class="content">
        <div class="left">
            <div class="left-hdr">\u{1F4CB} Sub-Tasks</div>
        ...
        <div class="right">
            <div class="tabs">
                <button class="tab active" onclick="selectTab('code')">\u{1F4BB} Code</button>
                <button class="tab" onclick="selectTab('logs')">\u{1F4CB} Logs</button>
            </div>
