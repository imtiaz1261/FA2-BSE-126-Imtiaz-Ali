...
1. `index.html` ko browser mein kholein, ya local server ke saath `http://localhost:8000` par kholein.
2. Header mein **Set Groq key** par click karein aur apni valid Groq key paste karein.
3. Key sirf current browser session mein rehti hai; source files mein save nahi hoti.

Agar browser security ki wajah se local files (`file://`) se external libraries load nahi hoti, folder ke andar simple local server chalayein:
```
